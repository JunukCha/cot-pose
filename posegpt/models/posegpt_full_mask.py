    ...
    def forward(
        ...
        **kwargs
        ...
        if labels is not None:
            ...
            shift_labels = labels[..., 1:].clone()            # (B, L-1), corresponds to orig[1:L]

            B, Lm1 = shift_labels.shape
            V = shift_logits.size(-1)

            loss_fct = CrossEntropyLoss(ignore_index=IGNORE_INDEX)
            target_seq1 = torch.tensor([733, 28748, 16289, 28793, 28705]).cuda() # [/INST]
            target_seq2 = torch.tensor([733, 28748, 16289, 28793]).cuda() # [/INST]
            updated_labels = shift_labels.clone()
            for b in range(B):
                shift_label = shift_labels[b]
                pos_b = (shift_label==self.pose_begin_idx).nonzero(as_tuple=True)[0]
                pos_e = (shift_label==self.pose_end_idx).nonzero(as_tuple=True)[0]
                if pos_b.numel() and pos_e.numel():
                    pb = pos_b[0].item()  # index of pose_begin in orig
                    pe = pos_e[0].item()  # index of pose_end in orig
                    updated_labels[b, pb+2:pe+1] = updated_labels[b, pb+1:pe].clone()
                    updated_labels[b, pb+1] = -100

                found = False
                # found target_seq1 '[/INST]' for ablation 3
                for i in range(shift_label.shape[0] - len(target_seq1) + 1):
                    if torch.equal(shift_label[i:i+len(target_seq1)], target_seq1):
                        updated_labels[b, :i+len(target_seq1)] = -100
                        found = True
                        break
                # found target_seq2 '[/INST]'
                if not found:
                    for i in range(shift_label.shape[0] - len(target_seq2) + 1):
                        if torch.equal(shift_label[i:i+len(target_seq2)], target_seq2):
                            updated_labels[b, :i+len(target_seq2)] = -100
                            break
            ...
            # loss_text = loss_fct(
            #     shift_logits[:, :pb+1].permute(0, 2, 1),        # (B*(L-1), V)
            #     updated_labels[:, :pb+1]                 # (B*(L-1),)
            # )
            # loss_pose = loss_fct(
            #     shift_logits[:, pb+1:].permute(0, 2, 1),        # (B*(L-1), V)
            #     updated_labels[:, pb+1:]                 # (B*(L-1),)
            # )
            # weight = 0.8
            # loss = weight*loss_text+weight*loss_pose
            # # 1st loss (loss1): compute CE loss on shift_labels up to and including the position of '<pose_begin>',
            # # then ignore everything after '<pose_begin>' by setting to IGNORE_INDEX

            # labels1 = shift_labels.clone()  # (B, L-1)
            # # input_ids1 = input_ids.clone() # For debug
            # for b in range(B):
            #     orig = labels[b]  # (L,)   # original labels for this batch entry
            #     # find the index of '<pose_begin>' in orig
            #     pos_b = (orig == self.pose_begin_idx).nonzero(as_tuple=True)[0]
            #     if pos_b.numel():
            #         pb = pos_b[0].item()           # position of pose_begin in orig
            #         pb_shift = pb - 1              # corresponding index in shift_labels
            #         # mask everything after pb_shift (exclusive) by setting to IGNORE_INDEX
            #         if pb_shift + 1 < Lm1:
            #             labels1[b, pb_shift+1:] = IGNORE_INDEX
            #             # input_ids1[b, pb_shift+1:] = IGNORE_INDEX

            # loss1 = loss_fct(
            #     shift_logits.view(-1, V),        # (B*(L-1), V)
            #     labels1.view(-1)                 # (B*(L-1),)
            # )

            # # ─────────────────────────────────────────────────────────────────────────────
            # # 2nd loss (loss2): compute CE loss on original labels (not shift and aligned with input_ids)
            # # from '<pose_begin>' (not including) tokens up to just before '<pose_end>' (not including)

            # labels2 = torch.full_like(shift_labels, IGNORE_INDEX)  # (B, L-1), initialize all as IGNORE
            # # input_ids2 = input_ids.clone() # For debug
            # for b in range(B):
            #     orig = labels[b]  # (L,)
            #     # find indices of '<pose_begin>' and '<pose_end>' in orig
            #     pos_b = (orig == self.pose_begin_idx).nonzero(as_tuple=True)[0]
            #     pos_e = (orig == self.pose_end_idx).nonzero(as_tuple=True)[0]
            #     if pos_b.numel() and pos_e.numel():
            #         pb = pos_b[0].item()  # index of pose_begin in orig
            #         pe = pos_e[0].item()  # index of pose_end in orig

            #         # if there is a valid interval between pb+1 and pe-1
            #         if pb + 1 < pe:
            #             # copy tokens from orig[pb+1:pe] into labels2 for CE loss
            #             labels2[b, pb+1:pe] = orig[pb+1:pe]
            #             # input_ids2[b, :pb+1] = IGNORE_INDEX
            #             # input_ids2[b, pe:] = IGNORE_INDEX

            # loss2 = loss_fct(
            #     shift_logits.view(-1, V),   # (B*(L-1), V)
            #     labels2.view(-1)            # (B*(L-1),)
            # )
            
            # labels3 = shift_labels.clone()  # (B, L-1)
            # # input_ids3 = input_ids.clone() # For debug
            # for b in range(B):
            #     orig = labels[b]  # (L,)   # original labels for this batch entry
            #     # find the index of '<pose_end>' in orig
            #     pos_e = (orig == self.pose_end_idx).nonzero(as_tuple=True)[0]
            #     if pos_e.numel():
            #         pe = pos_e[0].item()           # position of pose_end in orig
            #         pe_shift = pe - 1              # corresponding index in shift_labels
            #         # mask everything after pb_shift (exclusive) by setting to IGNORE_INDEX
            #         if pe_shift + 1 < Lm1:
            #             labels3[b, :pe_shift+1] = IGNORE_INDEX
            #             # input_ids3[b, :pe_shift+1] = IGNORE_INDEX
